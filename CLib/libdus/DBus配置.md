...
* /etc/dbus-1目录下:system-local.conf或session-local.conf
...
interface/org.gnome.Shell.Screenshot.xml
```xml
<!DOCTYPE node PUBLIC
...
<node>
  ...
  <interface name="org.gnome.Shell.Screenshot">
    ...
    <!--
        ...
        indicating whether the operation was successful or not.
 50         @filename can either be an absolute path or a basename, in
        which case the screenshot will be saved in the $XDG_PICTURES_DIR
        or the home directory if it doesn't exist. The filename used
        to save the screenshot will be returned in @filename_used.
    -->
...
services/org.gtk.vfs.Daemon.service
```ini
[D-BUS Service]
...
```xml
<!-- keep in sync with dirs/dirs.go:SetRootDir() -->
<busconfig>
  <servicedir>/var/lib/snapd/dbus-1/services/</servicedir>
</busconfig>
```

system.d/snapd.session-services.conf
```xml
<!-- keep in sync with dirs/dirs.go:SetRootDir() -->
<busconfig>
  <servicedir>/var/lib/snapd/dbus-1/system-services/</servicedir>
</busconfig>
```
...
* type:system/session
* include:
* includedir:目录下的.conf后缀名的文件会被include
* user:
* fork:显示,会成为守护进程 --fork
* keep_umask:
* syslog:log到syslog; --syslog, --syslog-only and --nosyslog
* pidfile:把pid把到特定的文件; --nopidfile
* allow_anonymous
* listen
* auth
* servicedir
